...
- [Broadcast Multi Access Network (DR, BDR, DROther)](#broadcast-multi-access-network-dr-bdr-drother)
  ...
  - [Solutions to OSPF broadcast multi-access problems (`DR`, `BDR`, `DROther`)](#solutions-to-ospf-broadcast-multi-access-problems-dr-bdr-drother)
...
# Broadcast Multi Access Network (DR, BDR, DROther)
...
- Without a DR:
  ...
  - Every router would form a full adjacency with every other router

  - Number of adjacencies = N(N-1)/2

  - For 10 routers = 45 adjacencies

- With a DR:

    - All routers form a full adjacency only with the DR (and BDR)

    - Much less LSDB synchronization traffic

    - OSPF scales better

## Solutions to OSPF broadcast multi-access problems (`DR`, `BDR`, `DROther`)

- `DR`: the leader of the OSPF segment

  - Receives LSAs from other routers

  - Floods LSAs to all routers on the segment

  - Forms full adjacency with all routers

- `BDR`: the backup leader

    - If the DR dies, new BDR is then elected
    
    - Maintains a synchronized LSDB

    - Ready to take over immediately if DR fails
...
- `DROther`: any router that is neither DR nor BDR
    ...
    - Does not flood LSAs directly to all routers

    - Forms full adjacency only with DR and BDR
